<script lang="ts">
    ...
    console.log(updatedTableData, 'average');
</script>
...
{#if chartData.length > 0}
    ...
    >
                            ...
                            <thead>
                                <tr class="w-[100%]  border border-secondary-100 dark:border-surface-700">
                                    {#each tableHeaders as header}
                                        <th
                                            class="w-[30%] py-3 pl-4 pr-3 text-left text-sm font-semibold text-primary-500 dark:text-primary-100 sm:pl-3  border border-secondary-100 dark:border-surface-700"
                                        >
                                            {header}
                                        </th>
                                    {/each}
                                ...
                                {#each updatedTableData as data}
                                    <tr class="hover:bg-secondary-50 dark:hover:bg-surface-800 transition  border border-secondary-100 dark:border-surface-700">
                                        {#each Object.values(data).slice(0, -1) as value}
                                            <td
                                                class="whitespace-nowrap text-sm px-3 py-2 text-primary-500 dark:text-primary-100  border border-secondary-100 dark:border-surface-700"
                                            >
                                                {value || 'Unknown'}
                                            </td>
                                        {/each}
        ...
        <div class="p-4">
            <div
                class="flex col-span-3 overflow-x-auto justify-center items-center dark:border-surface-700 sm:px-4"
            >
                <Doughnut
                    data={chartData}
                    labels={chartLabels}
...
{:else}
    ...
    >
        <p
            class="text-xl flex text-center items-center justify-center place-items-center place-content-center"
